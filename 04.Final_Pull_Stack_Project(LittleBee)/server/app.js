/** 최종
 * yarn add axios body-parser cookie-parser cors dayjs dotenv express express-session express-useragent method-override multer node-schedule node-thumbnail nodemailer serve-favicon serve-static winston winston-daily-rotate-file express-mysql-session mysql2 
 */
...
const methodOverride = require('method-override');  // post 파라미터 처리
...
app.use((req, res, next) => {
    ...
    if (current_url.indexOf('jpg') > -1 || current_url.indexOf('jpeg') > -1 || current_url.indexOf('png') > -1 || current_url.indexOf('css') > -1 || current_url.indexOf('js') > -1) {
        return next();
    }
    ...
    // 클라이언트 접속이 종료된 경우의 이벤트 --> 모든 응답의 전송이 완료된 경우
    res.on('finish', () => {
        // 접속 종료시간
...
// body-parder를 이용해 application/x-www-form-urlencoded 파싱
...
app.use(methodOverride('X-Method-Override'));   // IBN

/** 쿠키를 처리할 수 있는 객체 연결 */
// cookie-parser는 데이터를 저장, 조회 할 떄 암호화 처리를 동반한다.
// 이 떄 암호화에 사용되는 key문자열을 개발자가 정해야 한다.
app.use(cookieParser(process.env.COOKIE_ENCRYPT_KEY))

/** HTML, CSS, IMG, JS 등의 정적 파일을 URL에 노출시킬 폴더 연결 */
// "http://아이피(혹은 도메인):포트번호" 이 후의 경로가 router에 등록되지 않은 경로라면
...
| 5) 각 URL별 백엔드 기능 정의 
-----------------------------------------------------------*/
/** 분리된 컨트롤러들을 Express객체에 등록 */
app.use(require('./controllers/UserController'));
app.use(require('./controllers/ProductController'));
app.use(require('./controllers/BasketController'));
app.use(require('./controllers/NoticeController'));
app.use(require('./controllers/PaymentController'));

/** 컨트롤러에서 에러 발생 시 next(에러객체)를 호출 했을 때 동작할 처리
 * (반드시 모든 route 처리의 맨 마지막에 위치해야 함)
 */
